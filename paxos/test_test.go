...
func TestManyForget(t *testing.T) {
	...
	go func() {
		na := rand.Perm(maxseq)
		fmt.Printf("Length of na %d \n", len(na))
		for i := 0; i < len(na); i++ {
			seq := na[i]
			j := (rand.Int() % npaxos)
			v := rand.Int()
			fmt.Printf("srv :%d  with proposa # %d \n", j, seq)
			pxa[j].Start(seq, v)
			fmt.Printf("srv :%d  with proposa # %d \n", j, seq)
			runtime.Gosched()
		}
...
func TestForgetMem(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		...
		//fmt.Printf("big is: %s", string(big))
		pxa[0].Start(i, string(big))
		//pxa[0].Start(i, "string dassdjcsklcnsklncsknc")
		waitn(t, pxa, i, npaxos)
	}
	...
	fmt.Printf("size m2 is: %d\n", m2.Alloc/1024/1024)
	fmt.Printf("size m1 is: %d\n", m1.Alloc/1024/1024)
	...
	for seq := 0; seq < npaxos && seq < 10; seq++ {
		for i := 0; i < npaxos; i++ {
			...
			fmt.Printf("fate on srv %d for seq %d is %d with value %v\n", i, seq, fate, v)
			fmt.Printf("fate is %d \n", Forgotten)
			if fate != Forgotten || v == again[seq] {
				t.Fatalf("2nd seq %d < Min() %d but not Forgotten", seq, pxa[i].Min())
			}
		}
	}
