...
func (cfg *config) checkCommittedIndex(index int, command interface{}, expectedServers int) {
	...
	//fmt.Printf("index %d cmd %d init %t \n", index, cmd, cmdInitialized)
...
func (cfg *config) replicateOne(server int, cmd int, expectedServers int) (
	...
	//	fmt.Printf("inside replicateOne %d \n", cmd)
	var pri *PBServer
	cfg.mu.Lock()
	...
	//	fmt.Printf("index value to be replicated %d \n", index)
	t0 := time.Now()
	for time.Since(t0).Seconds() < 10 {
		committed := pri.IsCommitted(index)
		if committed {
			...
			if nReplicated >= expectedServers {
				fmt.Printf("completed Replicate one for index %d & cmd %d \n", index, cmd)
				return index
			}
		}
...
func (cfg *config) viewChange(newView int) {
	...
	for time.Since(t0).Seconds() < 5 {
		...
		if ok && view >= newView {
			fmt.Printf("view %d completed & primary %d status: %d with curr_view is %d and log %v @ time: %s\n ", newView, pri.me, pri.status, pri.currentView, pri.log, time.Now().String())
			return
		}
		fmt.Printf("view %d primary %d status: %d with currView %d @ %s \n", newView, pri.me, pri.status, pri.currentView, time.Now().String())
		time.Sleep(80 * time.Millisecond)
	}
	//	view, ok := pri.ViewStatus()
	//	if ok {
	//		fmt.Printf("ok")
	//	}
	//fmt.Printf("view %d not completed and current view is %d %v \n ", newView, view, pri)
